...
local mytextclock =
  wibox.widget.textclock("<span font='Terminus 5'> </span>%H:%M ")
  ...
  settings = function()
    if mpd_now.state == "play" then
      ...
      artist = " "
        ...
        .. markup(
          "#777777",
          " <span font='Terminus 2'> </span>|<span font='Terminus 5'> </span>"
        )
      mpdicon:set_image(theme.play)
    elseif mpd_now.state == "pause" then
      title = "mpd "
      artist = "paused"
        .. markup("#777777", " |<span font='Terminus 5'> </span>")
...
local batupd = lain.widget.bat({
  settings = function()
    if
      not bat_now.status
      or bat_now.status == "N/A"
      or type(bat_now.perc) ~= "number"
    then
      return
    end
...
local batbg =
  wibox.container.background(batbar, "#474747", gears.shape.rectangle)
